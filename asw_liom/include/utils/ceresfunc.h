...
struct ResidualBlockInfo
{
	...
	std::vector<double *> parameter_blocks; //优化变量内存地址数组
...
//			ROS_WARN("release marginlizationinfo");
	...
	void addResidualBlockInfo(ResidualBlockInfo *residual_block_info){
		...
		for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size()); i++)
		{
			...
			// 形成map reinterpret_cast强制转换，从指针类型转换为long类型
	...
	void preMarginalize(){
		for (auto it : factors)
		{
			...
			for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
			{
				...
				// 判断是否已经存在相同地址的参数块数据。如果不存在，则继续执行下面的代码
				if (parameter_block_data.find(addr) == parameter_block_data.end())
				{
					double *data = new double[size];
					// 将 it 对象中的参数块数据复制到新分配的内存中
	...
	void marginalize(){
		...
		{
			// 如果在parameter_block_idx中没有找到，就认为是不用marg的变量
		...
		//in marginalize pos size: 30 marg size(m): 15 left size(n): 15
		//in marginalize pos size: 60 marg size(m): 45 left size(n): 15
		std::cout << "in marginalize pos size: "<< pos << " marg size(m): " << m << " left size(n): " << n << std::endl;
	...
	int m, n;
	std::unordered_map<long, int> parameter_block_size; // <地址，数量>
	int sum_block_size;
	// 需要被marg掉的优化变量的id
	std::unordered_map<long, int> parameter_block_idx; // <地址，id>
	std::unordered_map<long, double *> parameter_block_data; // <地址，指向数据的地址>
...
struct Cost_Initial_G
{
	...
	bool operator()( const T *q, T *residual) const {
		...
		Eigen::Matrix<T, 3, 1> resi = q_wg * g_I - acc_T; //残差
		residual[0] = resi[0];
		residual[1] = resi[1];
		residual[2] = resi[2];
...
struct Cost_Initialization_Prior_R
{
	...
	bool operator()( const T *r_wg_, T *residual) const {
		...
		Sophus::SO3<T> SO3_R_wg = Sophus::SO3<T>::exp(R_wg); // rotation vector to transform matrix

		Eigen::Matrix<T, 3, 1> prior_T(prior.cast<T>());
		Sophus::SO3<T> prior_R_wg = Sophus::SO3<T>::exp(prior_T);

		Sophus::SO3<T> d_R = SO3_R_wg.inverse() * prior_R_wg;
		Eigen::Matrix<T, 3, 1> d_Phi = d_R.log();           // To tangent space rotation vector
