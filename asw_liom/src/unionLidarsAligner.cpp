...
class LidarsParamEstimator{
    private:
        ...
        // publish the raw horizon point cloud
        ...
        const uint64 delt_time = 1644348482864430380; // forest01 1644348482.864430380
        // const uint64 delt_time = 1645959893327394970; // indoor01 1645959893.327394970
        // const uint64 delt_time = 1644348482824525880; // forest02 1644348482.824525880
        // const uint64 delt_time = 0;
        // int delt_time;
    ...
    public:
        ...
        {
            sub_velo = nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_points", 1000, 
                                            &LidarsParamEstimator::velo_cloud_handler, this);
            // sub_velo = nh.subscribe<sensor_msgs::PointCloud2>("/points_raw", 1000, 
            //                                 &LidarsParamEstimator::velo_cloud_handler, this);
            sub_hori = nh.subscribe<livox_ros_driver::CustomMsg>("/livox/lidar", 100, 
                                            &LidarsParamEstimator::hori_cloud_handler, this);
            sub_imu  = nh.subscribe("/livox/imu", 2000,  &LidarsParamEstimator::imu_handler, this);
            // sub_imu  = nh.subscribe("/imu_raw", 2000,  &LidarsParamEstimator::imu_handler, this);
            ...
            pub_time_union_cloud = nh.advertise<union_cloud::union_cloud>("/a_time_union_cloud", 1);
            ...
            // if (!private_nh_.getParam("delt_time", delt_time)) delt_time = 0;
            ...
            // ROS_INFO_STREAM( "delt_time     : " <<  delt_time );
        ...
        {
            std::cout << "get hori timestamp: " << std::setprecision(15) <<livox_msg_in->header.stamp.toSec() << std::endl;
            ...
            if(_first_hori){
                ...
                _hori_start_stamp = livox_msg_in->header.stamp.toNSec() + delt_time;
            ...
            uint64 temp_time = livox_msg_in_distort.header.stamp.toNSec() + delt_time;
            std::cout << livox_msg_in_distort.header.stamp.toNSec() << " + " << delt_time << " = " << temp_time << std::endl;
            livox_msg_in_distort.header.stamp = ros::Time().fromNSec(temp_time);
            livox_msg_in_distort.timebase = temp_time;
                ...
                {  //publish pointcloud
                    ...
                    if(_cut_raw_message_pieces == 1 && _velo_queue.size() > 2){
                        ...
                        if(pub_horipoints_given_stamp( velo_start_stamp, velo_end_stamp, livox_aligned_msg)){
                            ...
                            union_cloud::union_cloud a_time_union;
        ...
        {
            std::cout << "get velo timestamp: " << std::setprecision(15)<<  pointCloudIn->header.stamp.toSec() << std::endl;
            // ros::Time v_stamp;
            // v_stamp.fromNSec(pointCloudIn->header.stamp.toNSec());
            // pcl::PointCloud<PointType>  v_cloud;
            // pcl::fromROSMsg(*pointCloudIn, v_cloud);
            // Eigen::Matrix4f temp_trans_mat;
            // temp_trans_mat << 0.997885,  0.0607353,  0.0231535, -0.0743897,
            //                                     -0.0604807,   0.998103, -0.0115404, 0.00176412,
            //                                     -0.0238104,  0.0101157,   0.999665,   0.139614,
            //                                     0,          0,          0,          1;

            // // temp_trans_mat << 9.999976e-01, 7.553071e-04, -2.035826e-03, -8.086759e-01,
            // //                               -7.854027e-04, 9.998898e-01, -1.482298e-02, 3.195559e-01,
            // //                               2.024406e-03, 1.482454e-02, 9.998881e-01,  -7.997231e-01,
            // //                               0.0, 0.0, 0.0,  1.0;
            // pcl::transformPointCloud (v_cloud , v_cloud, temp_trans_mat);
            // // std::cout << "_velo_hori_tf_matrix: \n" << _velo_hori_tf_matrix << std::endl;
            ...
            float endOri = -atan2(full_cloud_in.points[cloudSize - 1].y,
                    full_cloud_in.points[cloudSize - 1].x) + 2 * M_PI;
            ...
            for (int i = 0; i < cloudSize; i++)
            {
                ...
                // calculate the time relative to the first point
        ...
        void transform_hori_timestamp()
        {
            ...
            for(int i = 0; i < hori_msg_vec.size(); i++)
            {
                ...
                std::cout << " i:" << i << " | delta_t: " << delta_t << " -> " 
                          << ros::Time().fromNSec( delta_t).toSec()
        ...
        bool pub_horipoints_given_stamp( uint64 velo_start_stamp, 
                                         uint64 velo_end_stamp, 
                                         livox_ros_driver::CustomMsg &livox_aligned_msg)
        {
